...
export function createSendWhatsAppTool(
  ...
  return createTool({
    description: isScheduled
      ? "Send a WhatsApp message to the customer. You are running as an automated scheduled task — send directly without waiting for approval. Supports optional image attachment via imageUrl."
      : "Send a WhatsApp message to the customer. IMPORTANT: Always show the draft to the user and wait for explicit approval before calling this tool. Never send without confirmation. Supports optional image attachment via imageUrl.",
    ...
    execute: async (ctx, input) => {
      ...
      try {
        ...
        const idempotencyKey = `${customerId}:${phone}:${await hashBody(input.body)}:${today}`;
